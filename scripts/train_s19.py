"""
train_s19.py — Sprint 19: BiLSTM S19 — augmentation agresivo para generalización entre señantes

Diferencias vs S18:
  Augmentation   : noise σ 0.008→0.020, flip 0.50→0.70, scale ±15% (nuevo)
                   time-warp 0.40→0.60, feature-drop 0.30→0.50
  Label smoothing: 0.05 → 0.15 (reduce sobreajuste a señantes)
  Objetivo       : F1-test >= 0.30 con HE3 PASA

Arquitectura:   proj(150→128) → LayerNorm → BiLSTM(128,256) → TemporalAttention → head
Checkpoints:    bilstm_s19.pt / bilstm_s19.onnx
"""
...
print(f"Sprint 19 — BiLSTM S19 | LSP peruano + AEC | per-sample norm | min_muestras={MIN_SAMPLES}")
print("=" * 65)
...
def load_dataset():
    ...
    print(f"\nDataset S19 (LSP + AEC, min15, aug-agresivo):")
...
class SignDataset(Dataset):
    ...
    def __getitem__(self, i):
        x = self.X[i].copy()
        if self.augment:
            # Noise más fuerte para simular variación entre señantes
            x = x + np.random.randn(*x.shape).astype(np.float32) * 0.020
            # Scale augment ±15%: simula diferencias de tamaño corporal
            if np.random.rand() < 0.60:
                scale = np.random.uniform(0.85, 1.15)
                x = x * scale
            # Flip horizontal más frecuente
            if np.random.rand() < 0.70:
                x[:, 66:87]   = -x[:, 66:87]
                x[:, 108:129] = -x[:, 108:129]
            # Time-warp más frecuente
            if np.random.rand() < 0.60:
                x = time_warp(x, sigma=0.06)
            # Feature drop más agresivo
...
class LSPLSTMBidirS19(nn.Module):
...
# ════════════════════════════════════════════════════════════════════════════
# HPO BiLSTM S19 (warm-start S13 best, 20 trials)
# ════════════════════════════════════════════════════════════════════════════
...
if SKIP_HPO:
    ...
    print(f"\n{'='*65}")
    print("BLOQUE A — HPO BiLSTM S19  (warm-start S13, 20 trials)")
    print(f"{'='*65}")
    ...
    def objective(trial):
        ...
        def make():
            return LSPLSTMBidirS19(N_DIMS, n_classes,
...
print(f"\nB. KFold({N_FOLDS}) final BiLSTM S19")
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S19-test")

best_fold_idx = int(np.argmax(fold_f1s))
last_va = list(skf.split(X_core, y_core))[best_fold_idx][1]
res_cal = full_eval(best_model, X_core[last_va], y_core[last_va], "BiLSTM-S19-cal")
...
# ONNX
lat = export_onnx(best_model, "bilstm_s19.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s19.pt")
print(f"  ✅ checkpoints/bilstm_s19.pt")

# Guardar label maps como JSON para otros scripts
with open(DATA_DIR / "s19_label2idx.json", "w", encoding="utf-8") as f:
    json.dump(label2idx, f, ensure_ascii=False, indent=2)
with open(DATA_DIR / "s19_idx2label.json", "w", encoding="utf-8") as f:
    json.dump({str(k): v for k, v in idx2label.items()}, f, ensure_ascii=False, indent=2)
print(f"  ✅ data/s19_label2idx.json  ({n_classes} LSP - Vocabulario-palabras)")
...
row = {
    ...
    "notas":       (f"Dataset S19 LSP+AEC min15 aug-agresivo {len(X_all)} muestras {n_classes} clases "
...
print(f"\n{'='*65}")
print("[TRAIN S19 COMPLETADO]")
print(f"{'='*65}")
print(f"  BiLSTM S19  F1-test : {res_te['f1_macro']:.4f}")
