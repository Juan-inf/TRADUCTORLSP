"""
train_s17.py — Sprint 17: BiLSTM S17 — n_layers=1 + min_muestras=15 (HE3 fix)

Diferencias vs S16:
  Dataset        : dataset_s15.npz (mismos datos)
  min_muestras   : 15 (101 clases bien pobladas)
  n_layers       : 1 (S15 con n_layers=1 pasó HE3; S16 con n_layers=2 falló)
  HPO            : skip-hpo, S13_BEST params
  Objetivo       : F1-test >= 0.35 con HE3 PASA

Arquitectura:   proj(150→128) → LayerNorm → BiLSTM(128,256) → TemporalAttention → head
Checkpoints:    bilstm_s17.pt / bilstm_s17.onnx
"""
...
print(f"Sprint 17 — BiLSTM S17 | LSP peruano + AEC | per-sample norm | min_muestras={MIN_SAMPLES}")
print("=" * 65)
...
def load_dataset():
    ...
    print(f"\nDataset S17 (LSP peruano + AEC, min15, n_layers=1):")
...
class LSPLSTMBidirS17(nn.Module):
...
# ════════════════════════════════════════════════════════════════════════════
# HPO BiLSTM S17 (warm-start S13 best, 20 trials)
# ════════════════════════════════════════════════════════════════════════════
...
if SKIP_HPO:
    ...
    print(f"\n{'='*65}")
    print("BLOQUE A — HPO BiLSTM S17  (warm-start S13, 20 trials)")
    print(f"{'='*65}")
    ...
    def objective(trial):
        ...
        def make():
            return LSPLSTMBidirS17(N_DIMS, n_classes,
...
print(f"\nB. KFold({N_FOLDS}) final BiLSTM S17")
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S17-test")

best_fold_idx = int(np.argmax(fold_f1s))
last_va = list(skf.split(X_core, y_core))[best_fold_idx][1]
res_cal = full_eval(best_model, X_core[last_va], y_core[last_va], "BiLSTM-S17-cal")
...
# ONNX
lat = export_onnx(best_model, "bilstm_s17.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s17.pt")
print(f"  ✅ checkpoints/bilstm_s17.pt")

# Guardar label maps como JSON para otros scripts
with open(DATA_DIR / "s17_label2idx.json", "w", encoding="utf-8") as f:
    json.dump(label2idx, f, ensure_ascii=False, indent=2)
with open(DATA_DIR / "s17_idx2label.json", "w", encoding="utf-8") as f:
    json.dump({str(k): v for k, v in idx2label.items()}, f, ensure_ascii=False, indent=2)
print(f"  ✅ data/s17_label2idx.json  ({n_classes} LSP - Vocabulario-palabras)")
...
row = {
    ...
    "notas":       (f"Dataset S17 LSP+AEC min15 n1 {len(X_all)} muestras {n_classes} clases "
...
print(f"\n{'='*65}")
print("[TRAIN S17 COMPLETADO]")
print(f"{'='*65}")
print(f"  BiLSTM S17  F1-test : {res_te['f1_macro']:.4f}")
