"""train_s36.py — Sprint 36: dataset_s35 (S18 + s31_continuo + s34_eaf,
filtrado sin IX), presupuesto de entrenamiento completo (KFold(5), 60
épocas) — mismo salto que S32→S33 pero para esta línea.

S35 (train_s35.py, split único rápido, 30 épocas) dio un resultado
NEGATIVO: WER peor que S33 en los dos benchmarks (1.086 vs 1.027 en los
5 videos oficiales; 1.188 vs 1.062 en las 10 oraciones ELAN nuevas). Antes
de descartar la fuente s34_eaf del todo, se corrigen dos cosas y se repite
con presupuesto completo:

  1. Se excluye la glosa "IX" (señalamiento pronominal de forma visual
     variable, no una seña léxica fija — ver build_dataset_s35_merge.py)
     que era ~17% de las muestras de s34_eaf y probable ruido de etiqueta.
  2. Se usa el presupuesto completo (KFold(5) + 60 épocas/paciencia 10) en
     vez del split único recortado de S35 — el mismo salto que ya mejoró
     WER de S32 (1.134, split único) a S33 (1.027, KFold completo) sobre
     un dataset de tamaño casi idéntico.

Si con estas dos correcciones el WER sigue sin bajar de 1.027, la
conclusión honesta sería que s34_eaf no aporta con el volumen actual de
datos (503 muestras/37 clases tras el filtro), independientemente del
presupuesto de entrenamiento — no que falte más presupuesto.

Checkpoints: bilstm_s36.pt / bilstm_s36.onnx
"""
...
from sklearn.model_selection import StratifiedShuffleSplit, GroupShuffleSplit, StratifiedKFold
...
# Presupuesto completo (ver docstring del módulo) — el recorte de S32 dejó margen sin usar
N_EPOCHS_FINAL = 60
PATIENCE_FINAL = 10
N_FOLDS        = 5
...
print("Sprint 36 — BiLSTM S36 | dataset_s35 (S18 + s31_continuo + s34_eaf sin IX), presupuesto completo KFold(5)")
print("=" * 65)
...
def load_dataset():
    ...
    print(f"\nDataset S35 (S18 + s31_continuo + s34_eaf sin IX):")
...
print(f"\nKFold({N_FOLDS}) — {N_EPOCHS_FINAL} épocas máx, paciencia {PATIENCE_FINAL}:")
...
f1_val = float(np.mean(fold_f1s))
f1_val_std = float(np.std(fold_f1s))
print(f"\n  KFold F1: {f1_val:.4f} ± {f1_val_std:.4f}  ({total_min:.1f} min)")

print("\nEvaluación en test holdout:")
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S36-test")
...
lat = export_onnx(best_model, "bilstm_s36.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s36.pt")
print(f"  ✅ checkpoints/bilstm_s36.pt")

fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s36",
    "sprint":      "S36",
    "modelo":      "BiLSTM-S36",
    "features":    "pose+rhand+lhand(150dims×30fr)+per-sample-zscore+s18+s31continuo+s34eaf_sinIX-merge",
    "hp_resumen":  (f"hidden={hp['hidden']},n_layers={hp['n_layers']},"
                    f"dropout={hp['dropout']:.2f},lr={hp['lr']:.2e}"),
    "f1_val_mean": f"{f1_val:.4f}",
    "f1_val_std":  f"{f1_val_std:.4f}",
    ...
    "notas":       (f"Dataset S35 (S18 raw + s31_continuo + s34_eaf sin IX, cross-source group fix extendido) "
                    f"{len(X_all)} muestras {n_classes} clases per-sample-zscore aug-agresivo; "
                    f"skip-hpo S13-best, KFold({N_FOLDS}) presupuesto completo (vs split único de S32); "
                    ...
                    f"objetivo real: WER sobre 5 videos + 10 oraciones ELAN reservadas, ver scripts/evaluar_wer_s36.py"),
}
...
print(f"\n{'='*65}")
print("[TRAIN S36 COMPLETADO]")
print(f"{'='*65}")
print(f"  F1-test    : {res_te['f1_macro']:.4f}  (274 clases — no comparable 1:1 con producción 96 clases)")
print(f"  Top-5 test : {res_te['top5']:.4f}")
print(f"  HE3        : {'PASA' if he3['passed'] else 'FALLA'}  ΔF1={he3['delta_f1']:.4f}")
print(f"  Siguiente paso: scripts/evaluar_wer_s36.py")
