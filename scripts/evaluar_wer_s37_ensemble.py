...
def secuencia_predicha_ensemble(frames_kp, sess_a, i2l_a, sess_b, i2l_b,
                                 vocab_comun, clase_a_idx):
    segmentador = SegmentadorPausas()
    segmentos = []
    for kp in frames_kp:
...
def main():
    ...
    resultados = {"s33": [], "s36": [], "ensemble_s33_s36": [], "produccion": []}
    ...
    for entry in holdout:
        ...
        ref_s36 = secuencia_real(glosas, vocab_s36)
        hyp_s36 = secuencia_predicha_un_modelo(frames_kp, sess_s36, i2l_s36, vocab_s36)
        w_s36 = wer(ref_s36, hyp_s36)
        print(f"  [S36] ref(n={len(ref_s36)}): {ref_s36}")
        print(f"  [S36] hyp: {hyp_s36}   WER={w_s36}")
        if w_s36 is not None:
            resultados["s36"].append(w_s36)

        ref_ens = secuencia_real(glosas, set(vocab_comun_36))
        hyp_ens = secuencia_predicha_ensemble(frames_kp, sess_s33, i2l_s33, sess_s36, i2l_s36,
                                               vocab_comun_36, clase_a_idx_36)
        w_ens = wer(ref_ens, hyp_ens)
        print(f"  [ens33+36] ref(n={len(ref_ens)}): {ref_ens}")
        print(f"  [ens33+36] hyp: {hyp_ens}   WER={w_ens}")
    ...
    with open(DATA / "s37_ensemble_wer_resultados.json", "w", encoding="utf-8") as f:
        json.dump({
