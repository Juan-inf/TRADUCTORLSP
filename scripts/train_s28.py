"""
train_s28.py — Sprint 28: BiLSTM sobre dataset_s18 (grupos cross-source corregidos)

Diferencias vs S26:
  Dataset    : dataset_s18.npz — igual a S16 pero con grupos unificados para
...
Checkpoints : bilstm_s28.pt / bilstm_s28.onnx
"""
...
print(f"Sprint 28 — BiLSTM S28 | dataset_s18 (grupos cross-source fix) | min={MIN_SAMPLES}")
print("=" * 65)
...
def load_dataset():
    npz_path = DATA_DIR / "dataset_s18.npz"
    if not npz_path.exists():
        raise FileNotFoundError(
            "dataset_s18.npz no encontrado.\n"
    ...
    with open(DATA_DIR / "s18_label2idx.json", encoding="utf-8") as f:
    ...
    print(f"\nDataset S28 (S18 — + glosas reales de narrativas):")
...
lat = export_onnx(best_model, "bilstm_s28.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s28.pt")
print(f"  ✅ checkpoints/bilstm_s28.pt")

with open(DATA_DIR / "s28_label2idx.json", "w", encoding="utf-8") as f:
    json.dump(label2idx, f, ensure_ascii=False, indent=2)
with open(DATA_DIR / "s28_idx2label.json", "w", encoding="utf-8") as f:
    json.dump({str(k): v for k, v in idx2label.items()}, f, ensure_ascii=False, indent=2)
print(f"  ✅ data/s28_label2idx.json  ({n_classes} clases)")

fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s28",
    "sprint":      "S28",
    "modelo":      "BiLSTM-S28",
    "features":    "pose+rhand+lhand(150dims×30fr)+per-sample-zscore+s18-glosas-narrativas",
    ...
    "notas":       (f"Dataset S18 (+glosas reales) {len(X_all)} muestras {n_classes} clases "
