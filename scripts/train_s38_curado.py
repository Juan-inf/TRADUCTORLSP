"""train_s38_curado.py — Sprint 38: BiLSTM sobre dataset_s38_curado.npz,
vocabulario reducido a las 24 letras del abecedario (única porción del
vocabulario de 96 clases que llega a >=100 muestras de entrenamiento SIN
incluir clases narrativas inválidas HISTORIAS_VINETAS_*, ver
build_dataset_s38_curado.py).

Contexto: el checkpoint v4 (96 clases completas) mide F1=0.6107 sobre
este mismo subconjunto de 24 letras cuando se restringe la EVALUACIÓN
post-hoc (scripts/medir_f1_subconjunto_curado.py). Hipótesis de este
sprint: si el softmax del modelo NUNCA compite contra las otras 72 clases
(muchas de cola larga, <30 muestras), el F1 sobre estas 24 letras debería
subir por encima de 0.6107 y potencialmente cruzar 0.70 — quitar
competencia de gradiente por clases ruidosas, no agregar datos nuevos.

Advertencia de alcance (comunicada al usuario antes de entrenar): este
experimento mide específicamente la tarea "reconocer el abecedario LSP",
no "traducir vocabulario LSP a texto castellano" en general — es un
subconjunto legítimo pero angosto del objetivo OE1 original.

Checkpoints: bilstm_s38_curado.pt / bilstm_s38_curado.onnx
"""
...
print("Sprint 38 — BiLSTM S38-curado | dataset_s38_curado (24 letras del abecedario)")
print("=" * 65)
...
def load_dataset():
    npz_path = DATA_DIR / "dataset_s38_curado.npz"
    if not npz_path.exists():
        raise FileNotFoundError(
            "dataset_s38_curado.npz no encontrado.\n"
    ...
    with open(DATA_DIR / "s38_label2idx.json", encoding="utf-8") as f:
    ...
    print(f"\nDataset S38-curado (24 letras del abecedario):")
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S38-curado-test")
...
lat = export_onnx(best_model, "bilstm_s38_curado.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s38_curado.pt")
print(f"  ✅ checkpoints/bilstm_s38_curado.pt")

fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s38_curado",
    "sprint":      "S38-curado",
    "modelo":      "BiLSTM-S38-curado",
    "features":    "pose+rhand+lhand(150dims×30fr)+per-sample-zscore+vocabulario-curado-24letras",
    ...
    "notas":       (f"Dataset S38-curado (24 letras abecedario, >=100 muestras train, sin HISTORIAS_VINETAS_*) "
...
print(f"\n{'='*65}")
print("[TRAIN S38-CURADO COMPLETADO]")
print(f"{'='*65}")
print(f"  F1-test    : {res_te['f1_macro']:.4f}  (24 clases — SOLO abecedario, no comparable 1:1 con producción 96 clases)")
print(f"  Top-5 test : {res_te['top5']:.4f}")
print(f"  HE3        : {'PASA' if he3['passed'] else 'FALLA'}  ΔF1={he3['delta_f1']:.4f}")
print(f"  Siguiente paso: comparar F1-test con umbral OE1 (0.70) y con F1=0.6107 post-hoc de v4")
