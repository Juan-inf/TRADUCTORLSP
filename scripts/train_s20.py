"""
train_s20.py — Sprint 20: DANN (Domain Adversarial Neural Network)

Diferencias vs S19:
  DANN          : GradientReversalLayer + DomainClassifier sobre grupo-señante
  alpha_schedule: 0 (warmup 10 ep) → ALPHA_MAX=1.0 (formula Ganin et al. 2016)
  lambda_d      : 0.30 (peso de domain loss)

Hipotesis: si el feature extractor no puede predecir el grupo-señante, las
           representaciones aprendidas son invariantes al estilo personal
           → el modelo generaliza a señantes no vistos en HE3.

Arquitectura:   proj(150→128) → LN → BiLSTM(128,256) → TempAttn
                  ↳ ClassHead(512→256→101)
                  ↳ GRL(alpha) → DomainHead(512→64→N_grupos)

Checkpoints:    bilstm_s20.pt / bilstm_s20.onnx
"""
...
# S13 best — n_layers=1 fue estable, ls=0.15 reduce overfit a señantes
S13_BEST = {
    "hidden": 256, "n_layers": 1, "dropout": 0.20,
    "lr": 1.709e-3, "wd": 1.296e-4, "ls": 0.15,
}
LAMBDA_D    = 0.30   # peso de domain loss en la loss total
ALPHA_MAX   = 1.0    # scale máximo del gradient reversal
WARMUP_EP   = 10     # epochs de warmup puro clasificación antes de activar DANN
...
print(f"Sprint 20 — BiLSTM DANN | LSP+AEC | per-sample norm | min_muestras={MIN_SAMPLES}")
print("=" * 65)
...
def load_dataset():
    ...
    print(f"\nDataset S20 (LSP+AEC, min{MIN_SAMPLES}, DANN):")
...
# ── Dominios DANN ─────────────────────────────────────────────────────────────
unique_train_groups = sorted(set(g_core.tolist()))
group2domain        = {g: i for i, g in enumerate(unique_train_groups)}
n_domains           = len(unique_train_groups)
domain_core         = np.array([group2domain[int(g)] for g in g_core], dtype=np.int64)
print(f"  DANN: {n_domains} grupos-señante en core → dominios 0..{n_domains-1}")
print(f"  DANN: lambda_d={LAMBDA_D}  alpha_max={ALPHA_MAX}  warmup={WARMUP_EP}ep")
...
class SignDatasetDANN(Dataset):
    def __init__(self, X, y, domain=None, augment=False):
        self.X       = X.copy()
        self.y       = torch.from_numpy(y).long()
        self.domain  = torch.from_numpy(domain).long() if domain is not None else None
    ...
    def __getitem__(self, i):
        ...
        xi = torch.from_numpy(x).float()
        di = self.domain[i] if self.domain is not None else torch.tensor(0, dtype=torch.long)
        return xi, self.y[i], di


def make_loaders_dann(X_tr, y_tr, d_tr, X_val, y_val, batch=BATCH):
    cc  = Counter(y_tr.tolist())
    w   = [1.0 / cc[int(c)] for c in y_tr]
    smp = WeightedRandomSampler(w, num_samples=len(w), replacement=True)
    dl_tr  = DataLoader(SignDatasetDANN(X_tr, y_tr, domain=d_tr, augment=True),
                        batch_size=batch, sampler=smp, num_workers=0)
    dl_val = DataLoader(SignDatasetDANN(X_val, y_val, augment=False),
...
class LSPBiLSTM_DANN(nn.Module):
    def __init__(self, n_dims, n_classes, n_domains,
                 hidden=256, n_layers=1, dropout=0.20):
        super().__init__()
        ...
        # Domain discriminator (aplicado tras GRL durante training)
    ...
    def forward(self, x, alpha=None):
        ...
        if alpha is not None:           # training: retorna también domain logits
            rev = GradientReversalFn.apply(ctx, alpha)
            return cls_logits, self.domain_head(rev)
        return cls_logits               # inference: solo clasificación


class InferenceWrapper(nn.Module):
    """Wrapper para ONNX export: extrae solo el head de clasificación."""
    def __init__(self, model):
        super().__init__()
        self.model = model

    def forward(self, x):
        return self.model(x)  # alpha=None → cls_logits solamente
...
def dann_alpha(epoch, n_epochs):
    """DANN alpha schedule (Ganin 2016): 0 durante warmup, luego rampa hasta ALPHA_MAX."""
    if epoch <= WARMUP_EP:
        return 0.0
...
def train_one_run_dann(X_tr, y_tr, d_tr, X_val, y_val, model_fn,
                       lr, wd, label_smoothing, n_epochs, patience, trial=None):
    dl_tr, dl_val = make_loaders_dann(X_tr, y_tr, d_tr, X_val, y_val)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for xb, yb, db in dl_tr:
            ...
            optimizer.zero_grad()
            cls_logits, domain_logits = model(xb, alpha=alpha)
            cls_loss    = criterion(cls_logits, yb)
            domain_loss = domain_crit(domain_logits, db)
            loss = cls_loss + LAMBDA_D * domain_loss
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
        scheduler.step()

        # Validación solo sobre clasificación (sin domain)
        model.eval()
        preds, trues = [], []
        with torch.no_grad():
            for xb, yb, _ in dl_val:
                out = model(xb.to(DEVICE))  # alpha=None → cls solamente
...
def full_eval(model, X_eval, y_eval, label="eval"):
    ...
    dl = DataLoader(SignDatasetDANN(X_eval, y_eval, augment=False),
                    batch_size=BATCH, shuffle=False, num_workers=0)
    all_logits, all_true = [], []
    for xb, yb, _ in dl:
...
# ════════════════════════════════════════════════════════════════════════════
# HPO (skip — usando S13_BEST con ls=0.15)
# ════════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════════
# KFold(5) final con DANN
# ════════════════════════════════════════════════════════════════════════════

print(f"\nB. KFold({N_FOLDS}) final BiLSTM S20 DANN")


def make_best():
    return LSPBiLSTM_DANN(
        N_DIMS, n_classes, n_domains,
        hidden=hp["hidden"], n_layers=hp["n_layers"], dropout=hp["dropout"],
    )


print(f"  Params: {count_params(make_best()):,}  (incluyendo domain head)")
...
for fi, (tr_i, va_i) in enumerate(skf.split(X_core, y_core), 1):
    d_tr_fold = domain_core[tr_i]
    m, f1 = train_one_run_dann(
        X_core[tr_i], y_core[tr_i], d_tr_fold,
        X_core[va_i], y_core[va_i],
        model_fn=make_best,
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S20-test")

best_fold_idx = int(np.argmax(fold_f1s))
last_va       = list(skf.split(X_core, y_core))[best_fold_idx][1]
res_cal = full_eval(best_model, X_core[last_va], y_core[last_va], "BiLSTM-S20-cal")
...
# ── ONNX ─────────────────────────────────────────────────────────────────────
lat = export_onnx(best_model, "bilstm_s20.onnx")

# ── Checkpoint ────────────────────────────────────────────────────────────────
ckpt = {
    "model_state":      {k: v.cpu() for k, v in best_model.state_dict().items()},
    "architecture":     "LSPBiLSTM_DANN",
...
torch.save(ckpt, CKPT_DIR / "bilstm_s20.pt")
print(f"  ✅ checkpoints/bilstm_s20.pt")

with open(DATA_DIR / "s20_label2idx.json", "w", encoding="utf-8") as f:
    json.dump(label2idx, f, ensure_ascii=False, indent=2)
with open(DATA_DIR / "s20_idx2label.json", "w", encoding="utf-8") as f:
    json.dump({str(k): v for k, v in idx2label.items()}, f, ensure_ascii=False, indent=2)
print(f"  ✅ data/s20_label2idx.json  ({n_classes} clases LSP)")

# ── CSV log ───────────────────────────────────────────────────────────────────
fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s20",
    "sprint":      "S20",
    "modelo":      "BiLSTM-S20-DANN",
    "features":    "pose+rhand+lhand(150dims×30fr)+per-sample-zscore+DANN",
    "hp_resumen":  (f"hidden={hp['hidden']},n_layers={hp['n_layers']},"
                    f"dropout={hp['dropout']:.2f},lr={hp['lr']:.2e},"
                    f"lambda_d={LAMBDA_D},n_domains={n_domains}"),
    ...
    "notas":       (f"DANN min{MIN_SAMPLES} lambda_d={LAMBDA_D} alpha_max={ALPHA_MAX} "
                    f"warmup={WARMUP_EP}ep n_domains={n_domains}; "
                    f"aug-agresivo (noise+scale+flip+twarp+drop); "
...
# ── Resumen ───────────────────────────────────────────────────────────────────
print(f"\n{'='*65}")
print("[TRAIN S20 COMPLETADO]")
print(f"{'='*65}")
print(f"  BiLSTM S20 DANN  F1-test : {res_te['f1_macro']:.4f}")
print(f"  BiLSTM S19       F1-test : 0.3357  (referencia aug-agresivo)")
print(f"  BiLSTM S13       F1-test : 0.3696  (mejor hasta S13)")
...
print(f"  HE3 holdout F1           : {he3['f1_ghold']:.4f}  (S19: 0.0260)")
