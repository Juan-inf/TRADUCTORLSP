"""
train_s22.py — Sprint 22: Body-Centered Correcto (sin re-normalización en augmentation)

Diagnóstico S21: la re-normalización body-centered DENTRO del augmentation
  propagaba el ruido del nodo nariz a todos los 148 features restantes
  (noise + re-center(noisy_nose) → ruido correlacionado → colapso de entrenamiento).

Solución S22:
  - Preprocesamiento: body-centered (restar nariz + z-score global) → igual que S21
  - Augmentation: noise/scale/flip/warp/drop → SIN re-normalizar al final
  - El modelo recibe features body-relative con augmentación directa (sin corrupción)

Checkpoints:    bilstm_s22.pt / bilstm_s22.onnx
"""
...
print(f"Sprint 22 — BiLSTM body-centered(correcto) | LSP+AEC | min_muestras={MIN_SAMPLES}")
print("=" * 65)


# ── Normalización body-centered (solo preprocesamiento) ───────────────────────

def normalize_body_centered(x: np.ndarray) -> np.ndarray:
    """
    Normalización relativa al cuerpo — usada SOLO en preprocesamiento inicial.

    1. Restar posición de la nariz (landmark #0) a todos los coords por frame.
    2. Z-score global para normalizar escala.

    Layout features por frame (150 dims):
...
def load_dataset():
    ...
    print(f"\nDataset S22 (LSP+AEC, min{MIN_SAMPLES}, body-centered):")
...
# ── Augmentation — SIN re-normalización al final ──────────────────────────────
...
class SignDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.augment:
            ...
            # Flip horizontal en espacio body-centered (nariz ≈ 0 → flip correcto)
            ...
            # ← NO re-normalizar: evita propagación de ruido desde nose a todos los features
...
print(f"\nB. KFold({N_FOLDS}) final BiLSTM S22")


def make_best():
    return LSPBiLSTMS22(N_DIMS, n_classes,
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S22-test")

best_fold_idx = int(np.argmax(fold_f1s))
last_va       = list(skf.split(X_core, y_core))[best_fold_idx][1]
res_cal = full_eval(best_model, X_core[last_va], y_core[last_va], "BiLSTM-S22-cal")
...
lat = export_onnx(best_model, "bilstm_s22.onnx")

ckpt = {
    "model_state":      {k: v.cpu() for k, v in best_model.state_dict().items()},
    "architecture":     "LSPBiLSTMS22",
    "normalize_input":  "body_centered_no_reagment_renorm",
    "label2idx":        label2idx,
    "idx2label":        idx2label,
    "n_classes":        n_classes,
    "n_dims":           N_DIMS,
...
torch.save(ckpt, CKPT_DIR / "bilstm_s22.pt")
print(f"  ✅ checkpoints/bilstm_s22.pt")

with open(DATA_DIR / "s22_label2idx.json", "w", encoding="utf-8") as f:
    json.dump(label2idx, f, ensure_ascii=False, indent=2)
with open(DATA_DIR / "s22_idx2label.json", "w", encoding="utf-8") as f:
    json.dump({str(k): v for k, v in idx2label.items()}, f, ensure_ascii=False, indent=2)
print(f"  ✅ data/s22_label2idx.json  ({n_classes} clases LSP)")

fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s22",
    "sprint":      "S22",
    "modelo":      "BiLSTM-S22-bodycenter-correct",
    "features":    "pose+rhand+lhand(150dims×30fr)+body-centered(no-reagment-renorm)",
    ...
    "notas":       (f"body-centered min{MIN_SAMPLES} aug-agresivo SIN renorm; "
...
print(f"\n{'='*65}")
print("[TRAIN S22 COMPLETADO]")
print(f"{'='*65}")
print(f"  BiLSTM S22 body-ctr  F1-test : {res_te['f1_macro']:.4f}")
print(f"  BiLSTM S21 body+reN  F1-test : 0.0833  (S21 con bug renorm)")
...
print(f"  HE3 holdout F1               : {he3['f1_ghold']:.4f}  (S19:0.026 S20:0.012 S21:0.005)")
