"""
train_s26.py — Sprint 26: BiLSTM sobre dataset_s16 (91 clases, grupos mejorados)

Diferencias vs S19:
  Dataset    : dataset_s16.npz (91 clases, 12 048 muestras, estrategia por_subgrupo)
  Grupos     : abecedario → 3 sub-grupos por letra (fix HE3 66% holdout F1=0)
               aec        → 5 sub-grupos (2 281 muestras expandidas desde RAR)
  HPs        : S13-best warm-start (hidden=256, n_layers=1, dropout=0.20,
               lr=1.709e-3, wd=1.296e-4, ls=0.15)
  Augment    : agresivo igual a S19 (noise σ=0.020, flip 70%, scale ±15%,
               time-warp 60%, feature-drop 50%)
...
Checkpoints : bilstm_s26.pt / bilstm_s26.onnx
"""
...
print(f"Sprint 26 — BiLSTM S26 | dataset_s16 (91 clases, grupos mejorados) | min={MIN_SAMPLES}")
print("=" * 65)
...
def load_dataset():
    npz_path = DATA_DIR / "dataset_s16.npz"
    if not npz_path.exists():
        raise FileNotFoundError(
            "dataset_s16.npz no encontrado.\n"
            "  python3 scripts/build_dataset_s16.py"
        )
    ...
    with open(DATA_DIR / "s16_label2idx.json", encoding="utf-8") as f:
    ...
    print(f"\nDataset S26 (S16 — 91 clases, grupos mejorados):")
...
# Diagnóstico: clases en holdout sin entrenamiento
...
class LSPLSTMBidirS26(nn.Module):
...
def per_class_f1(res, idx2label, top_n_worst=15):
    """Imprime F1 por clase, ordenado de peor a mejor (top_n_worst clases)."""
    ...
    return dict(zip([idx2label.get(c, str(c)) for _, c in pairs],
                    [float(s) for s, _ in pairs]))
...
def he3_eval(model, res_test, X_ghold, y_ghold, T_opt):
    ...
    # F1 por clase en group-holdout
    print()
    pcf1 = per_class_f1(res_gh, idx2label, top_n_worst=20)
...
# ════════════════════════════════════════════════════════════════════════════
# BLOQUE A — HPO (warm-start S13-best)
# ════════════════════════════════════════════════════════════════════════════
...
if SKIP_HPO:
    ...
    print(f"\n{'='*65}")
    print("BLOQUE A — HPO BiLSTM S26  (warm-start S13-best, 20 trials)")
    print(f"{'='*65}")
    ...
    def objective(trial):
        ...
        def make():
            return LSPLSTMBidirS26(N_DIMS, n_classes,
...
print(f"\nB. KFold({N_FOLDS}) final BiLSTM S26")
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S26-test")

best_fold_idx = int(np.argmax(fold_f1s))
last_va = list(skf.split(X_core, y_core))[best_fold_idx][1]
res_cal = full_eval(best_model, X_core[last_va], y_core[last_va], "BiLSTM-S26-cal")
...
lat = export_onnx(best_model, "bilstm_s26.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s26.pt")
print(f"  ✅ checkpoints/bilstm_s26.pt")

with open(DATA_DIR / "s26_label2idx.json", "w", encoding="utf-8") as f:
    json.dump(label2idx, f, ensure_ascii=False, indent=2)
with open(DATA_DIR / "s26_idx2label.json", "w", encoding="utf-8") as f:
    json.dump({str(k): v for k, v in idx2label.items()}, f, ensure_ascii=False, indent=2)
print(f"  ✅ data/s26_label2idx.json  ({n_classes} clases)")
...
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s26",
    "sprint":      "S26",
    "modelo":      "BiLSTM-S26",
    "features":    "pose+rhand+lhand(150dims×30fr)+per-sample-zscore+s16-grupos",
    ...
    "notas":       (f"Dataset S16 grupos-mejorados {len(X_all)} muestras {n_classes} clases "
                    f"min≥{MIN_SAMPLES} per-sample-zscore aug-agresivo; warmstart S13; "
...
print(f"\n{'='*65}")
print("[TRAIN S26 COMPLETADO]")
print(f"{'='*65}")
print(f"  BiLSTM S26  F1-test : {res_te['f1_macro']:.4f}")
print(f"  BiLSTM S19  F1-test : 0.3357  (referencia — dataset S15)")
