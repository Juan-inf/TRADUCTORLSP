"""
train_s30.py — Sprint 30: BiLSTM sobre dataset_s18bc (Fase 1, limpio) (grupos cross-source corregidos)

Diferencias vs S26:
  Dataset    : dataset_s18bc.npz — igual a S16 pero con grupos unificados para
...
Checkpoints : bilstm_s30.pt / bilstm_s30.onnx
"""
...
print(f"Sprint 30 — BiLSTM S30 | dataset_s18bc (grupos cross-source fix) | min={MIN_SAMPLES}")
print("=" * 65)
...
def load_dataset():
    npz_path = DATA_DIR / "dataset_s18bc.npz"
    if not npz_path.exists():
        raise FileNotFoundError(
            "dataset_s18bc.npz no encontrado.\n"
            "  python3 scripts/build_dataset_s18.py"
        )
    ...
    with open(DATA_DIR / "s18bc_label2idx.json", encoding="utf-8") as f:
    ...
    print(f"\nDataset S30 (S18bc — Fase 1 sin dgi156_gloss, ventanas fijas sospechosas):")
...
lat = export_onnx(best_model, "bilstm_s30.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s30.pt")
print(f"  ✅ checkpoints/bilstm_s30.pt")

with open(DATA_DIR / "s30_label2idx.json", "w", encoding="utf-8") as f:
    json.dump(label2idx, f, ensure_ascii=False, indent=2)
with open(DATA_DIR / "s30_idx2label.json", "w", encoding="utf-8") as f:
    json.dump({str(k): v for k, v in idx2label.items()}, f, ensure_ascii=False, indent=2)
print(f"  ✅ data/s30_label2idx.json  ({n_classes} clases)")

fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s30",
    "sprint":      "S30",
    "modelo":      "BiLSTM-S30",
    "features":    "pose+rhand+lhand(150dims×30fr)+per-sample-zscore+s18bc-fase1-sin-dgi156gloss",
    ...
    "notas":       (f"Dataset S18bc (Fase 1, sin dgi156_gloss) {len(X_all)} muestras {n_classes} clases "
