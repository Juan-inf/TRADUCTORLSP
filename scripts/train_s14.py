...
class LSPLSTMBidirS14(nn.Module):
...
print(f"\nB. KFold({N_FOLDS}) final BiLSTM S14")
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S14-test")

best_fold_idx = int(np.argmax(fold_f1s))
last_va = list(skf.split(X_core, y_core))[best_fold_idx][1]
res_cal = full_eval(best_model, X_core[last_va], y_core[last_va], "BiLSTM-S14-cal")
...
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s14",
    "sprint":      "S14",
    "modelo":      "BiLSTM-S14",
...
print(f"\n{'='*65}")
print("[TRAIN S14 COMPLETADO]")
print(f"{'='*65}")
print(f"  BiLSTM S14  F1-test : {res_te['f1_macro']:.4f}")
