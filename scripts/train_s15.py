"""
train_s15.py — Sprint 15: BiLSTM S15 — S14 + datos AEC (LSP Peruano)

Diferencias vs S14:
  Dataset        : dataset_s15.npz = S14 base + instancias AEC
                   13,408 muestras → 255 clases (vs 139 en S14)
                   +1,663 instancias AEC (53 clases overlap + nuevas clases)
  Normalización  : per-sample z-score (igual que S14)
  Warm-start     : S13 best params (igual que S14)
  Objetivo       : más datos AEC → mejorar F1 en clases débiles

Arquitectura:   proj(150→128) → LayerNorm → BiLSTM(128,256) → TemporalAttention → head
Checkpoints:    bilstm_s15.pt / bilstm_s15.onnx
"""
...
print(f"Sprint 15 — BiLSTM S15 | LSP peruano + AEC | per-sample norm | min_muestras={MIN_SAMPLES}")
print("=" * 65)
...
def load_dataset():
    ...
    print(f"\nDataset S15 (LSP peruano + AEC, normalizado):")
...
class LSPLSTMBidirS15(nn.Module):
...
# ════════════════════════════════════════════════════════════════════════════
# HPO BiLSTM S15 (warm-start S13 best, 20 trials)
# ════════════════════════════════════════════════════════════════════════════
...
if SKIP_HPO:
    ...
    print(f"\n{'='*65}")
    print("BLOQUE A — HPO BiLSTM S15  (warm-start S13, 20 trials)")
    print(f"{'='*65}")
    ...
    def objective(trial):
        ...
        def make():
            return LSPLSTMBidirS15(N_DIMS, n_classes,
...
print(f"\nB. KFold({N_FOLDS}) final BiLSTM S15")
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S15-test")

best_fold_idx = int(np.argmax(fold_f1s))
last_va = list(skf.split(X_core, y_core))[best_fold_idx][1]
res_cal = full_eval(best_model, X_core[last_va], y_core[last_va], "BiLSTM-S15-cal")
...
# ONNX
lat = export_onnx(best_model, "bilstm_s15.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s15.pt")
print(f"  ✅ checkpoints/bilstm_s15.pt")
...
print(f"  ✅ data/s15_label2idx.json  ({n_classes} LSP - Vocabulario-palabras)")
...
row = {
    ...
    "notas":       (f"Dataset S15 LSP+AEC {len(X_all)} muestras {n_classes} clases "
...
print(f"\n{'='*65}")
print("[TRAIN S15 COMPLETADO]")
print(f"{'='*65}")
print(f"  BiLSTM S15  F1-test : {res_te['f1_macro']:.4f}")
