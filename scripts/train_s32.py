"""train_s32.py — Sprint 32: BiLSTM sobre dataset_s32 (S18 raw + s31_continuo
combinados — ver scripts/build_dataset_s32_merge.py).

Camino recomendado en ENTREGABLE_PLAN_DE_DESPLIEGUE_S13.md §12.5 tras el
resultado negativo de S31 (entrenado aislado sobre solo 1,538 muestras,
perdió contra producción en WER real): combinar las muestras de contexto
narrativo (S31, vía SRT × landmarks) con el corpus grande de clips ya
aislados (mismas fuentes que dataset_s18), usando el mismo rebalanceo de
grupos cross-source ya validado en S17/S18, extendido para incluir la
nueva fuente "s31_continuo".

Vocabulario mucho más grande que producción (270 clases vs 96) — S28 ya
mostró que saltar a 216 clases de golpe diluye F1 (0.22) frente a
mantenerse en 96 (0.42+). Se acepta ese trade-off a propósito: el objetivo
de este sprint no es maximizar F1 de clasificación aislada, es bajar el
WER real medido en scripts/evaluar_wer_s31.py sobre los 5 videos narrativos
reservados — que necesitan reconocer vocabulario que el modelo de
producción (96 clases) ni siquiera tiene en su vocabulario.

Presupuesto de tiempo acotado a propósito — S28 (17,471 muestras, 216
clases, KFold(5), 100 épocas) tardó 65,225s (~18h). Con 20,689 muestras y
270 clases este dataset es aún más grande; para no repetir un runaway de
horas, se usa un solo split train/val (no KFold(5)) y menos épocas/paciencia.

Checkpoints: bilstm_s32.pt / bilstm_s32.onnx
"""
...
print("Sprint 32 — BiLSTM S32 | dataset_s32 (S18 + s31_continuo combinados)")
print("=" * 65)
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S32-test")
...
lat = export_onnx(best_model, "bilstm_s32.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s32.pt")
print(f"  ✅ checkpoints/bilstm_s32.pt")

fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s32",
    "sprint":      "S32",
    "modelo":      "BiLSTM-S32",
    ...
    "notas":       (f"Dataset S32 (S18 raw + s31_continuo merge, cross-source group fix extendido) "
                    ...
                    f"objetivo real: WER sobre 5 videos reservados, ver scripts/evaluar_wer_s31.py"),
}
...
print(f"  Siguiente paso: scripts/evaluar_wer_s31.py adaptado a bilstm_s32.onnx")
