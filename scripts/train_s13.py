...
Diferencias vs S12:
  ...
  Checkpoints   : bilstm_s13.pt / bilstm_s13.onnx
...
# S11 BiLSTM best params (warm-start)
S11_BEST = {
    "hidden": 256, "n_layers": 1, "dropout": 0.20,
    "lr": 3.09e-3, "wd": 1e-4, "ls": 0.10,
}
...
_ap.add_argument("--min-muestras", type=int, default=2,
                 help="Mínimo de muestras por clase (default: 2)")
...
def load_dataset():
    s13_path = DATA_DIR / "dataset_s13.npz"
    if not s13_path.exists():
        raise FileNotFoundError(
            "dataset_s13.npz no encontrado. Ejecutar primero:\n"
            "  .venv310/bin/python3 scripts/build_dataset_s13.py"
        )

    data   = np.load(s13_path)
    X      = data["X"]
    y_raw  = data["y"]
    groups = data["groups"]
    ...
    try:
        ...
        times  = [
            (lambda t0: (time.perf_counter() - t0) * 1000)(
                (sess.run(None, {"sequence": np_in}), time.perf_counter())[1])
            for _ in range(200)
        ]
        lat = float(np.mean(times))
        print(f"  ONNX → {onnx_p.name}  "
              f"({onnx_p.stat().st_size/1e6:.1f} MB)  lat={lat:.2f} ms")
        return lat
...
# ════════════════════════════════════════════════════════════════════════════
# HPO BiLSTM S12 (warm-start S11 best, 20 trials)
# ════════════════════════════════════════════════════════════════════════════

print(f"\n{'='*65}")
print("BLOQUE A — HPO BiLSTM S12  (warm-start S11, 20 trials)")
print(f"{'='*65}")
...
def objective(trial):
    ...
    def make():
        return LSPLSTMBidirS12(N_DIMS, n_classes,
...
study.enqueue_trial({
    "hidden":   S11_BEST["hidden"],   "n_layers": S11_BEST["n_layers"],
    "dropout":  S11_BEST["dropout"],  "lr":       S11_BEST["lr"],
    "wd":       S11_BEST["wd"],       "ls":       S11_BEST["ls"],
})
...
print(f"\nB. KFold({N_FOLDS}) final BiLSTM S12")
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S12-test")

# Calibración
best_fold_idx = int(np.argmax(fold_f1s))
last_va = list(skf.split(X_core, y_core))[best_fold_idx][1]
res_cal = full_eval(best_model, X_core[last_va], y_core[last_va], "BiLSTM-S12-cal")
...
# ONNX
lat = export_onnx(best_model, "bilstm_s13.onnx")

# Checkpoint
ckpt = {
    "model_state":    {k: v.cpu() for k, v in best_model.state_dict().items()},
    "architecture":   "LSPLSTMBidirS12",
    "label2idx":      label2idx,
    "idx2label":      idx2label,
    "n_classes":      n_classes,
    "n_dims":         N_DIMS,
...
torch.save(ckpt, CKPT_DIR / "bilstm_s13.pt")
print(f"  ✅ checkpoints/bilstm_s13.pt")

# ── Actualizar logs/runs.csv ──────────────────────────────────────────────────

fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s13",
    "sprint":      "S13",
    "modelo":      "BiLSTM-S12",
...
print(f"\n{'='*65}")
print("[TRAIN S12 COMPLETADO]")
print(f"{'='*65}")
print(f"  BiLSTM S12  F1-test : {res_te['f1_macro']:.4f}")
print(f"  BiLSTM S11  F1-test : 0.0554  (mejor S12))")
print(f"  LSTM S10    F1-test : 0.0302  (baseline)")
mejora_vs_s11 = (res_te['f1_macro'] / 0.0199 - 1) * 100
mejora_vs_s10 = (res_te['f1_macro'] / 0.0302 - 1) * 100
print(f"  Mejora vs S11       : {mejora_vs_s11:+.1f}%")
print(f"  Mejora vs S10       : {mejora_vs_s10:+.1f}%")
supera_s10 = "✅" if res_te['f1_macro'] > 0.0302 else "❌"
print(f"  ¿Supera S10?        : {supera_s10}")
