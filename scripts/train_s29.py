"""
train_s29.py — Sprint 29: BiLSTM sobre dataset_s18b (Fase 1) (grupos cross-source corregidos)

Diferencias vs S26:
  Dataset    : dataset_s18b.npz — igual a S16 pero con grupos unificados para
...
Checkpoints : bilstm_s29.pt / bilstm_s29.onnx
"""
...
print(f"Sprint 29 — BiLSTM S29 | dataset_s18b (grupos cross-source fix) | min={MIN_SAMPLES}")
print("=" * 65)
...
def load_dataset():
    npz_path = DATA_DIR / "dataset_s18b.npz"
    if not npz_path.exists():
        raise FileNotFoundError(
            "dataset_s18b.npz no encontrado.\n"
    ...
    with open(DATA_DIR / "s18b_label2idx.json", encoding="utf-8") as f:
    ...
    print(f"\nDataset S29 (S18b — Fase 1: densificación de las 96 clases actuales):")
...
lat = export_onnx(best_model, "bilstm_s29.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s29.pt")
print(f"  ✅ checkpoints/bilstm_s29.pt")

with open(DATA_DIR / "s29_label2idx.json", "w", encoding="utf-8") as f:
    json.dump(label2idx, f, ensure_ascii=False, indent=2)
with open(DATA_DIR / "s29_idx2label.json", "w", encoding="utf-8") as f:
    json.dump({str(k): v for k, v in idx2label.items()}, f, ensure_ascii=False, indent=2)
print(f"  ✅ data/s29_label2idx.json  ({n_classes} clases)")

fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s29",
    "sprint":      "S29",
    "modelo":      "BiLSTM-S29",
    "features":    "pose+rhand+lhand(150dims×30fr)+per-sample-zscore+s18b-fase1-densificacion",
    ...
    "notas":       (f"Dataset S18b (Fase 1, 96 clases densificadas) {len(X_all)} muestras {n_classes} clases "
