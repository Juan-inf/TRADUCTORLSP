"""
train_s21.py — Sprint 21: BiLSTM S21 — Normalización Body-Centered

Diferencias vs S19 (mejor F1 sin DANN):
  Normalización  : body-centered (restar posición nariz/frame) + z-score global
                   vs S19: z-score global directo
  Hipótesis      : distintos señantes aparecen en distintas posiciones del frame;
                   centrar en nariz hace los coords relativos al cuerpo → invariante al señante

Layout features (30 frames × 150 dims):
  pose_x[0..32]      → índices  0-32   (nariz = índice 0)
  pose_y[0..32]      → índices 33-65   (nariz = índice 33)
...
Checkpoints:    bilstm_s21.pt / bilstm_s21.onnx
"""
...
print(f"Sprint 21 — BiLSTM body-centered | LSP+AEC | min_muestras={MIN_SAMPLES}")
print("=" * 65)


# ── Normalización body-centered ───────────────────────────────────────────────

def normalize_body_centered(x: np.ndarray) -> np.ndarray:
    """
    Normalización relativa al cuerpo:
    1. Restar posición de la nariz (landmark #0) a todos los coords por frame.
    2. Z-score global para normalizar escala.

    Esto hace que las coordenadas sean relativas al cuerpo del señante,
    eliminando variación por posición en el frame y tamaño corporal.
    """
    out = x.copy()
    nose_x = out[:, 0:1]    # [T, 1] x de nariz por frame
    nose_y = out[:, 33:34]   # [T, 1] y de nariz por frame

    # Centrar coords x en nariz
    out[:, 0:33]    -= nose_x   # pose_x
    out[:, 66:87]   -= nose_x   # left_hand_x
    out[:, 108:129] -= nose_x   # right_hand_x

    # Centrar coords y en nariz
    out[:, 33:66]   -= nose_y   # pose_y
    out[:, 87:108]  -= nose_y   # left_hand_y
    out[:, 129:150] -= nose_y   # right_hand_y

    # Z-score global para normalizar escala
    mu  = out.mean()
    std = out.std()
    if std < 1e-8:
...
def normalize_batch(X: np.ndarray) -> np.ndarray:
    return np.stack([normalize_body_centered(X[i]) for i in range(len(X))])
...
def load_dataset():
    ...
    print("  Aplicando normalización body-centered (nariz como origen)...")
    ...
    print(f"\nDataset S21 (LSP+AEC, min{MIN_SAMPLES}, body-centered):")
...
class SignDataset(Dataset):
    ...
    def __getitem__(self, i):
        x = self.X[i].copy()
        if self.augment:
            # Añade ruido sobre las coordenadas body-centered
            x = x + np.random.randn(*x.shape).astype(np.float32) * 0.020
            # Escala ±15% sobre el espacio body-centered
            if np.random.rand() < 0.60:
                x = x * np.random.uniform(0.85, 1.15)
            # Flip horizontal en espacio body-centered (nariz en 0 → correcto)
            if np.random.rand() < 0.70:
                x[:, 66:87]   = -x[:, 66:87]    # left_hand_x
                x[:, 108:129] = -x[:, 108:129]   # right_hand_x
            if np.random.rand() < 0.60:
            ...
            # Re-normalizar: body-centered + z-score
            x = normalize_body_centered(x)
...
print(f"\nB. KFold({N_FOLDS}) final BiLSTM S21")


def make_best():
    return LSPBiLSTMS21(N_DIMS, n_classes,
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S21-test")

best_fold_idx = int(np.argmax(fold_f1s))
last_va       = list(skf.split(X_core, y_core))[best_fold_idx][1]
res_cal = full_eval(best_model, X_core[last_va], y_core[last_va], "BiLSTM-S21-cal")
...
lat = export_onnx(best_model, "bilstm_s21.onnx")

ckpt = {
    "model_state":      {k: v.cpu() for k, v in best_model.state_dict().items()},
    "architecture":     "LSPBiLSTMS21",
    "normalize_input":  "body_centered",
    "label2idx":        label2idx,
    "idx2label":        idx2label,
    "n_classes":        n_classes,
    "n_dims":           N_DIMS,
...
torch.save(ckpt, CKPT_DIR / "bilstm_s21.pt")
print(f"  ✅ checkpoints/bilstm_s21.pt")

with open(DATA_DIR / "s21_label2idx.json", "w", encoding="utf-8") as f:
    json.dump(label2idx, f, ensure_ascii=False, indent=2)
with open(DATA_DIR / "s21_idx2label.json", "w", encoding="utf-8") as f:
    json.dump({str(k): v for k, v in idx2label.items()}, f, ensure_ascii=False, indent=2)
print(f"  ✅ data/s21_label2idx.json  ({n_classes} clases LSP)")

fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s21",
    "sprint":      "S21",
    "modelo":      "BiLSTM-S21-bodycenter",
    "features":    "pose+rhand+lhand(150dims×30fr)+body-centered+zscore",
    ...
    "notas":       (f"body-centered(nariz) min{MIN_SAMPLES} aug-agresivo; "
...
print(f"\n{'='*65}")
print("[TRAIN S21 COMPLETADO]")
print(f"{'='*65}")
print(f"  BiLSTM S21 body-ctr  F1-test : {res_te['f1_macro']:.4f}")
print(f"  BiLSTM S19 global-z  F1-test : 0.3357  (referencia)")
...
print(f"  Δ vs S13                     : {(res_te['f1_macro']/0.3696-1)*100:+.1f}%")
print(f"  Top-3 test                   : {res_te['top3']:.4f}")
print(f"  Top-5 test                   : {res_te['top5']:.4f}")
print(f"  HE3 holdout F1               : {he3['f1_ghold']:.4f}  (S19: 0.0260, S20: 0.0118)")
