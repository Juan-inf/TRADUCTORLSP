...
def load_dataset():
    s12_path = DATA_DIR / "dataset_s12.npz"
    if not s12_path.exists():
        raise FileNotFoundError(
            "dataset_s12.npz no encontrado. Ejecutar primero:\n"
            "  .venv310/bin/python3 scripts/build_dataset_s12.py"
        )

    data   = np.load(s12_path)
    X      = data["X"]
    y_raw  = data["y"]
    groups = data["groups"]

    json_path = DATA_DIR / "s12_label2idx.json"
...
# ONNX
lat = export_onnx(best_model, "bilstm_s12.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s12.pt")
print(f"  ✅ checkpoints/bilstm_s12.pt")

# ── Actualizar logs/runs.csv ──────────────────────────────────────────────────

fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s12",
    "sprint":      "S12",
    "modelo":      "BiLSTM-S12",
...
print(f"  BiLSTM S11  F1-test : 0.0199  (mejor S11)")
