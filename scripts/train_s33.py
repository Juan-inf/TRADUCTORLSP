"""train_s33.py — Sprint 33: mismo dataset_s32.npz que S32, presupuesto de
entrenamiento completo (KFold(5), más épocas) en vez del split único
recortado por precaución de tiempo.

S32 (train_s32.py) usó un solo split train/val con 30 épocas/paciencia 6
porque el precedente de S28 (dataset de tamaño similar) había tardado
65,225s (~18h) con KFold(5)+100 épocas, y no queríamos otro runaway de
horas sin saber el costo real de este dataset. Resultado real de S32: **9.9
minutos** — la precaución fue correcta pero el recorte fue mucho más de lo
necesario. Además, la curva de F1-val de S32 seguía mejorando de forma
ruidosa hasta la época 25 sin señal clara de plateau/overfitting cuando se
acabó el presupuesto de 30 épocas — hay margen real sin explotar.

Este sprint usa el presupuesto completo ya validado en el resto del
proyecto: KFold(5) + 60 épocas/paciencia 10 (a mitad de camino entre el
recorte de S32 y el estándar de 100/15 de S27-S31, con margen de tiempo
conocido: ~5 folds × 60 épocas × ~20s/época ≈ 100 min).

Checkpoints: bilstm_s33.pt / bilstm_s33.onnx
"""
...
print("Sprint 33 — BiLSTM S33 | dataset_s32 (S18 + s31_continuo), presupuesto completo KFold(5)")
print("=" * 65)
...
def load_dataset():
    npz_path = DATA_DIR / "dataset_s32.npz"
    if not npz_path.exists():
        raise FileNotFoundError(
            "dataset_s32.npz no encontrado.\n"
            "  python3 scripts/build_dataset_s32_merge.py"
        )
    ...
    with open(DATA_DIR / "s32_label2idx.json", encoding="utf-8") as f:
    ...
    print(f"\nDataset S32 (S18 + s31_continuo combinados):")
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S33-test")
...
lat = export_onnx(best_model, "bilstm_s33.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s33.pt")
print(f"  ✅ checkpoints/bilstm_s33.pt")

fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s33",
    "sprint":      "S33",
    "modelo":      "BiLSTM-S33",
    "features":    "pose+rhand+lhand(150dims×30fr)+per-sample-zscore+s18+s31continuo-merge",
    ...
    "notas":       (f"Dataset S32 (S18 raw + s31_continuo merge, cross-source group fix extendido) "
                    ...
                    f"objetivo real: WER sobre 5 videos reservados, ver scripts/evaluar_wer_s32.py"),
}
...
print(f"\n{'='*65}")
print("[TRAIN S33 COMPLETADO]")
print(f"{'='*65}")
print(f"  F1-test    : {res_te['f1_macro']:.4f}  (270 clases — no comparable 1:1 con producción 96 clases)")
print(f"  Top-5 test : {res_te['top5']:.4f}")
print(f"  HE3        : {'PASA' if he3['passed'] else 'FALLA'}  ΔF1={he3['delta_f1']:.4f}")
print(f"  Siguiente paso: scripts/evaluar_wer_s32.py adaptado a bilstm_s33.onnx")
