"""
train_s16.py — Sprint 16: BiLSTM S16 — HPO para 255 clases + min_muestras=15

Diferencias vs S15:
  Dataset        : dataset_s15.npz (mismos datos)
  min_muestras   : 15 (vs 5 en S15) → filtra clases con muy pocos datos
  HPO            : 20 trials warm-start desde S15-best (n_layers=2)
  Objetivo       : F1-test >= 0.25 con HE3 PASA

Arquitectura:   proj(150→128) → LayerNorm → BiLSTM(128,256) → TemporalAttention → head
Checkpoints:    bilstm_s16.pt / bilstm_s16.onnx
"""
...
# S15 best params — warm-start para S16 (n_layers=2 para 255 clases)
S13_BEST = {
    "hidden": 256, "n_layers": 2, "dropout": 0.10,
    "lr": 9.243e-4, "wd": 2.047e-4, "ls": 0.05,
}
...
print(f"Sprint 16 — BiLSTM S16 | LSP peruano + AEC | per-sample norm | min_muestras={MIN_SAMPLES}")
print("=" * 65)
...
def load_dataset():
    ...
    print(f"\nDataset S16 (LSP peruano + AEC, min_muestras=15):")
...
class LSPLSTMBidirS16(nn.Module):
...
# ════════════════════════════════════════════════════════════════════════════
# HPO BiLSTM S16 (warm-start S13 best, 20 trials)
# ════════════════════════════════════════════════════════════════════════════
...
if SKIP_HPO:
    ...
    print(f"\n{'='*65}")
    print("BLOQUE A — HPO BiLSTM S16  (warm-start S13, 20 trials)")
    print(f"{'='*65}")
    ...
    def objective(trial):
        ...
        def make():
            return LSPLSTMBidirS16(N_DIMS, n_classes,
...
print(f"\nB. KFold({N_FOLDS}) final BiLSTM S16")
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S16-test")

best_fold_idx = int(np.argmax(fold_f1s))
last_va = list(skf.split(X_core, y_core))[best_fold_idx][1]
res_cal = full_eval(best_model, X_core[last_va], y_core[last_va], "BiLSTM-S16-cal")
...
# ONNX
lat = export_onnx(best_model, "bilstm_s16.onnx")
...
torch.save(ckpt, CKPT_DIR / "bilstm_s16.pt")
print(f"  ✅ checkpoints/bilstm_s16.pt")

# Guardar label maps como JSON para otros scripts
with open(DATA_DIR / "s16_label2idx.json", "w", encoding="utf-8") as f:
    json.dump(label2idx, f, ensure_ascii=False, indent=2)
with open(DATA_DIR / "s16_idx2label.json", "w", encoding="utf-8") as f:
    json.dump({str(k): v for k, v in idx2label.items()}, f, ensure_ascii=False, indent=2)
print(f"  ✅ data/s16_label2idx.json  ({n_classes} LSP - Vocabulario-palabras)")
...
row = {
    ...
    "notas":       (f"Dataset S16 LSP+AEC min15 {len(X_all)} muestras {n_classes} clases "
...
print(f"\n{'='*65}")
print("[TRAIN S16 COMPLETADO]")
print(f"{'='*65}")
print(f"  BiLSTM S16  F1-test : {res_te['f1_macro']:.4f}")
