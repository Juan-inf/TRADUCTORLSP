"""
train_s24.py — Sprint 24: Hand-Only Body-Centered features (84 dims)

Diagnóstico S23: velocity features dan F1-test=0.178 (muy bajo) pero ΔF1=0.166
(más cercano al umbral 0.15). Análisis del holdout reveló la causa raíz:

  - 7 clases con core_n=0 (letras A,E,L,N,T,X + HV_3): 66% del holdout
    → F1=0 garantizado sin importar el modelo
  - 10 HISTORIAS_VINETAS: cross-source (dgi156↔vineta), F1≈0.044 actualmente
  - 1 BIEN: 2 muestras, negligible

Para mejorar las 10 HISTORIAS_VINETAS (única parte mejorable), eliminamos los
66 features de pose (cara/cuerpo) que son específicos del señante y usamos solo:
  - Mano izquierda relativa a nariz: lhand_x - nose_x, lhand_y - nose_y (42 dims)
  - Mano derecha relativa a nariz:  rhand_x - nose_x, rhand_y - nose_y (42 dims)
  Total: 84 features por frame → [30, 84]

Ventajas de hand-only body-centered:
  - Sin features de cara/tamaño corporal del señante → más invariante entre personas
  - Manos relativas a nariz capturan la POSICIÓN DEL SIGNO respecto al cuerpo
  - Cross-source: si el signo se hace igual entre dgi156 y vineta, features similares

Arquitectura:   proj(84→128) → LN → BiLSTM(128,256) → TempAttn → head
Checkpoints:    bilstm_s24.pt / bilstm_s24.onnx
"""
...
SEED     = 42
N_FRAMES = 30
N_DIMS   = 84      # hand-only: lhand(42) + rhand(42), centradas en nariz
BATCH    = 64
...
print(f"Sprint 24 — BiLSTM HAND-ONLY body-centered | LSP+AEC | min_muestras={MIN_SAMPLES}")
print("=" * 65)


# ── Normalización: hand-only body-centered ────────────────────────────────────
# Layout features entrada [T, 150]:
#   pose_x[0..32]      → índices 0-32   (nariz = 0)
#   pose_y[0..32]      → índices 33-65  (nariz = 33)
#   left_hand_x[0..20] → índices 66-86
#   left_hand_y[0..20] → índices 87-107
#   right_hand_x[0..20]→ índices 108-128
#   right_hand_y[0..20]→ índices 129-149
#
# Salida [T, 84]: solo manos centradas en nariz

def hand_only_body_centered(x: np.ndarray) -> np.ndarray:
    """
    Extrae features de manos relativas a nariz → [T, 84].
    Elimina features de cara/cuerpo (pose 0-65) para reducir
    variación específica del señante.
    """
    nose_x = x[:, 0:1]    # [T, 1]
    nose_y = x[:, 33:34]  # [T, 1]

    lhand_x = x[:, 66:87]   - nose_x   # [T, 21] left_hand relativo a nariz
    lhand_y = x[:, 87:108]  - nose_y   # [T, 21]
    rhand_x = x[:, 108:129] - nose_x   # [T, 21] right_hand relativo a nariz
    rhand_y = x[:, 129:150] - nose_y   # [T, 21]

    out = np.concatenate([lhand_x, lhand_y, rhand_x, rhand_y], axis=-1)  # [T, 84]

    mu = out.mean(); std = out.std()
    if std < 1e-8:
        return out.astype(np.float32)
    return ((out - mu) / std).astype(np.float32)


def normalize_batch(X: np.ndarray) -> np.ndarray:
    return np.stack([hand_only_body_centered(X[i]) for i in range(len(X))])
...
def load_dataset():
    ...
    print("  Extrayendo manos relativas a nariz [T,150]→[T,84] + z-score global...")
    ...
    print(f"\nDataset S24 (LSP+AEC, min{MIN_SAMPLES}, hand-only body-centered):")
...
# ── Augmentation sobre hand-only body-centered features ──────────────────────
...
class SignDataset(Dataset):
    ...
    def __getitem__(self, i):
        x = self.X[i].copy()  # [30, 84] hand-only body-centered pre-procesado
        if self.augment:
            # Ruido en posiciones relativas de manos
            x = x + np.random.randn(*x.shape).astype(np.float32) * 0.025
            # Escala de manos ±20% (simula diferencias de tamaño de mano y distancia)
            if np.random.rand() < 0.65:
                x = x * np.random.uniform(0.80, 1.20)
            # Flip horizontal: negar x de AMBAS manos y swap izq↔der
            # [0:21]=lhand_x, [21:42]=lhand_y, [42:63]=rhand_x, [63:84]=rhand_y
            if np.random.rand() < 0.70:
                lx, ly = x[:, 0:21].copy(), x[:, 21:42].copy()
                rx, ry = x[:, 42:63].copy(), x[:, 63:84].copy()
                x[:, 0:21]  = -rx   # left ← negado right
                x[:, 21:42] =  ry
                x[:, 42:63] = -lx   # right ← negado left
                x[:, 63:84] =  ly
            ...
            # SIN re-normalización: features augmentados directos
...
print(f"\n{'='*65}")
print("BLOQUE A — HPO omitido (--skip-hpo): usando parámetros S13-best (HO Body-Centered)")
print(f"{'='*65}")
...
print(f"\nB. KFold({N_FOLDS}) final BiLSTM S24 — Hand-Only Body-Centered features")


def make_best():
    return LSPBiLSTMS24(N_DIMS, n_classes,
...
res_te = full_eval(best_model, X_te, y_te, "BiLSTM-S24-test")

best_fold_idx = int(np.argmax(fold_f1s))
last_va       = list(skf.split(X_core, y_core))[best_fold_idx][1]
res_cal = full_eval(best_model, X_core[last_va], y_core[last_va], "BiLSTM-S24-cal")
...
lat = export_onnx(best_model, "bilstm_s24.onnx")

ckpt = {
    "model_state":      {k: v.cpu() for k, v in best_model.state_dict().items()},
    "architecture":     "LSPBiLSTMS24",
    "normalize_input":  "hand_only_body_centered_84dims",
    "label2idx":        label2idx,
    "idx2label":        idx2label,
    "n_classes":        n_classes,
    "n_dims":           N_DIMS,
...
torch.save(ckpt, CKPT_DIR / "bilstm_s24.pt")
print(f"  ✅ checkpoints/bilstm_s24.pt")

with open(DATA_DIR / "s24_label2idx.json", "w", encoding="utf-8") as f:
    json.dump(label2idx, f, ensure_ascii=False, indent=2)
with open(DATA_DIR / "s24_idx2label.json", "w", encoding="utf-8") as f:
    json.dump({str(k): v for k, v in idx2label.items()}, f, ensure_ascii=False, indent=2)
print(f"  ✅ data/s24_label2idx.json  ({n_classes} clases LSP)")

fecha = datetime.date.today().strftime("%Y%m%d")
row = {
    "exp_id":      f"exp_{fecha}_bilstm_s24",
    "sprint":      "S24",
    "modelo":      "BiLSTM-S24-hand-only",
    "features":    "hand-only body-centered(84dims×30fr)+zscore",
    ...
    "notas":       (f"hand-only body-centered(84d) min{MIN_SAMPLES} aug-sin-renorm; "
...
print(f"\n{'='*65}")
print("[TRAIN S24 COMPLETADO]")
print(f"{'='*65}")
print(f"  BiLSTM S24 hand-only F1-test : {res_te['f1_macro']:.4f}")
print(f"  BiLSTM S23 velocity  F1-test : 0.1778  (velocity features)")
print(f"  BiLSTM S22 body-ctr  F1-test : 0.3163  (body-centered)")
print(f"  BiLSTM S19 global-z  F1-test : 0.3357  (mejor aug)")
print(f"  BiLSTM S13           F1-test : 0.3696  (mejor histórico)")
print(f"  Δ vs S23                     : {(res_te['f1_macro']/0.1778-1)*100:+.1f}%")
print(f"  Δ vs S19                     : {(res_te['f1_macro']/0.3357-1)*100:+.1f}%")
print(f"  Top-3 test                   : {res_te['top3']:.4f}")
print(f"  Top-5 test                   : {res_te['top5']:.4f}")
print(f"  HE3 holdout F1               : {he3['f1_ghold']:.4f}  (S19:0.026 S22:0.025 S23:0.012)")
